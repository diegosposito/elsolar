...
abstract class BasesfGuardForgotPasswordActions extends sfActions
{
 ...
 $message = Swift_Message::newInstance('ucu.edu.ar', 25)
          ->setFrom(sfConfig::get('app_sf_guard_plugin_default_from_email', 'sistemas@ucu.edu.ar'))
          ->setTo($this->form->user->email_address)
          ->setSubject('Recuperacion de Clave para '.$this->form->user->username)
...
$mensaje= $this->getPartial('sfGuardForgotPassword/send_request', array('user' => $this->form->user, 'forgot_password' => $forgotPassword));

mail($this->form->user->email_address, 'Datos de Cuenta Personal de usuario : '.$this->form->user->username, $mensaje,"FROM: informatica@ucu.edu.ar\nX-Mailer: PHP");
