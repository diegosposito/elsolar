...
$(document).ready(function(){
	...
	$('#fechacerttittramite').datepicker({
		showOn: "button",
		buttonImage: "<?php echo $sf_request->getRelativeUrlRoot(); ?>/images/calendar.gif",
		buttonImageOnly: true,
		dateFormat: 'dd-mm-yy',
	...
	<?php } ?>	
	<?php if ($emailucu==0 || $email==0){ ?>
	$('#botonConfirmarEmail').attr("disabled","disabled");
	<?php } ?>
	<?php if ($idalumno==0) { ?>
	...
	<?php } ?>
    if($('#documentacion_8').is(':checked')) {  
    	$('#fechacerttittramite').val("<?php echo $fechacerttittramite; ?>");
    	$('#fechacerttittramite').removeAttr('disabled');
    } else {  
    	$('#fechacerttittramite').val("");
    	$('#fechacerttittramite').attr('disabled','disabled');
    }  
   ...
   $('#provincianacimiento').focusin(function() {
    ...
    $('#documentacion_8').change(function(){
        if($('#documentacion_8').is(':checked')) {  
        	$('#fechacerttittramite').val(fecha);
        	$('#fechacerttittramite').removeAttr('disabled');
        } else {  
        	$('#fechacerttittramite').val("");
        	$('#fechacerttittramite').attr('disabled','disabled');
    ...
    $('#botonGuardarDocumentacion').click(function() {
		// guardar la informacion de documentacion del alumnos ingresada
   	    $.post("<?php echo url_for('alumnos/guardardocumentacion'); ?>",
    ...
    $('#botonConfirmarEmail').click(function() {
		// guardar la informacion de documentacion adicional del alumnos ingresada
   	    $.post("<?php echo url_for('alumnos/confirmaremail'); ?>",
   	    	{idpersona: $('#idpersona').val()},
   			function(data){
				$('#mensajeInfoPersonal').html(data);
				$('#botonConfirmarEmail').removeAttr('disabled');
   			}
   	   	);
		return false;
   	}); 
...
<div id="tabs">
	...
	<form action="" id="formGuardarInfoPersonal" method="post">
	  <table cellspacing="0" class="stats" width="100%">
	    ...
	    <tbody>
	      ...
	      <tr>
	        <td>
	        	<?php echo $form['idtipoinscripto']->renderLabel() ?>
	        </td>
	        <td colspan="3">
	        	<?php echo $form['idtipoinscripto']->render() ?>
	        </td>
	      </tr>		      
	      <tr>
	        <td>
	...
	<form action="" id="formGuardarDocumentacion" method="post">
	  <table cellspacing="0" class="stats" width="100%">  
	    ...
	    <tbody>
	      ...
	      <tr>
	        <td width="15%">
	        	<?php echo $form['fechacerttittramite']->renderLabel() ?>
	        </td>
	        <td>
	        	<?php echo $form['fechacerttittramite']->render() ?>
	        </td>
	      </tr>  		      
	      <tr>
	        <td width="15%">
	...
	<div id="tabs-5">
		...
		<tr><td>
		    ...
		    webcam.set_api_url('<?php echo url_for('alumnos/guardarfotografia'); ?>/idpersona/<?php echo $idpersona; ?>');
