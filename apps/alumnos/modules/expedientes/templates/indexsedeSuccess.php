...
<table cellspacing="0" class="stats" width="100%">
  ...
  <tbody>
    ...
    <tr>
      ...
      } elseif($errores[$expediente->getIdexpediente()]==6 or $errores[$expediente->getIdexpediente()]==9) {
      	$color = 'bgcolor="olive"';    
      ...
      <form id="formExpediente_<?php echo $expediente->getIdexpediente(); ?>" action="">
      	<input type="button" value="Ver" onclick="location.href='<?php echo url_for('expedientes/ver?idexpediente='.$expediente->getIdexpediente().'&credencial=sede') ?>'">
    	<?php if (($errores[$expediente->getIdexpediente()]==0) or ($errores[$expediente->getIdexpediente()]==5)) { ?>
    		...
    		<input type="button" value="Derivar" onclick="location.href='<?php echo url_for('derivaciones/derivar?idexpediente='.$expediente->getIdexpediente().'&credencial=sede') ?>'">
    	...
    	<?php } elseif ($errores[$expediente->getIdexpediente()]==6) { ?>
    		<input type="button" value="Archivar" onclick="location.href='<?php echo url_for('derivaciones/archivar?idexpediente='.$expediente->getIdexpediente().'&credencial=sede') ?>'">
    	<?php } elseif ($errores[$expediente->getIdexpediente()]==9) { ?>
...
<font color="indigo"><b>VIOLETA:</b></font> Hay observaciones de Biblioteca o Administración Central.