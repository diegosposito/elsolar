...
<br>
<?php 
if ($credencial=="auditoria") { 
	$url = 'expedientes/indexauditoria';
} elseif ($credencial=="titulos") {
	$url = 'expedientes/indextitulos';
} elseif ($credencial=="facultad") {
	$url = 'expedientes/indexfacultad';
} elseif ($credencial=="sede") {
	$url = 'expedientes/indexsede';
} else {
	$url = 'derivaciones/index';
}
?>
<?php include_partial('formDerivar', array('form' => $form, 'url' => $url)) ?>
<p align="center"><input type="button" value="Volver" onclick="location.href='<?php echo url_for($url) ?>'"></p>
<br>	